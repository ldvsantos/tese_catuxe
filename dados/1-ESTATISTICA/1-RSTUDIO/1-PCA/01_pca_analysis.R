...
extrair_dados_bib <- function(caminho_bib) {
  ...
  dados$NLP <- as.integer(grepl("natural language processing|nlp\\b|text mining|sentiment analysis", texto_completo))
  
  # Features: Conhecimentos Tradicionais
  dados$TraditionalKnowledge <- as.integer(grepl("traditional knowledge|indigenous knowledge|local knowledge", texto_completo))
  dados$Ethnobotany <- as.integer(grepl("ethnobotany|ethnobotanical|medicinal plants", texto_completo))
  dados$CulturalHeritage <- as.integer(grepl("cultural heritage|intangible heritage|cultural practices", texto_completo))
  dados$IndigenousPeoples <- as.integer(grepl("indigenous|native people|aboriginal|tribal", texto_completo))
  dados$AncestralKnowledge <- as.integer(grepl("ancestral|ancestral knowledge|traditional practices", texto_completo))
  
  # Features: Métodos Etnográficos
  dados$Ethnography <- as.integer(grepl("ethnography|ethnographic|field work|participant observation", texto_completo))
  dados$ParticipativeResearch <- as.integer(grepl("participatory|participative|community-based research|cbpr", texto_completo))
  dados$OralHistory <- as.integer(grepl("oral history|oral tradition|storytelling|narratives", texto_completo))
  dados$SemiStructuredInterview <- as.integer(grepl("interview|semi-structured|in-depth interview", texto_completo))
  dados$FocusGroup <- as.integer(grepl("focus group|group discussion|community meeting", texto_completo))
  
  # Features: Agroecologia
  dados$Agroecology <- as.integer(grepl("agroecology|agroecological|sustainable agriculture", texto_completo))
  dados$Biodiversity <- as.integer(grepl("biodiversity|biological diversity|agrobiodiversity", texto_completo))
  dados$Conservation <- as.integer(grepl("conservation|preserve|protect|sustainability", texto_completo))
  dados$TraditionalFarming <- as.integer(grepl("traditional farming|traditional agriculture|subsistence farming", texto_completo))
  dados$OrganicFarming <- as.integer(grepl("organic farming|organic agriculture|ecological farming", texto_completo))
  dados$Permaculture <- as.integer(grepl("permaculture|agroforestry|polyculture", texto_completo))
  
  # Features: Aplicações
  dados$KnowledgeMapping <- as.integer(grepl("knowledge mapping|knowledge documentation|knowledge preservation", texto_completo))
  dados$Classification <- as.integer(grepl("classification|classify|categorization", texto_completo))
  dados$PatternRecognition <- as.integer(grepl("pattern recognition|pattern analysis|data mining", texto_completo))
  dados$SpatialAnalysis <- as.integer(grepl("spatial analysis|gis|geographic information|mapping", texto_completo))
  dados$TextAnalysis <- as.integer(grepl("text analysis|content analysis|discourse analysis", texto_completo))
...
################################################################################
# FUNÇÃO: Gerar Biplot com Clusters Sobrepostos
################################################################################
plot_biplot_with_clusters <- function(pca_result, output_file = "pca_biplot_clusters.png") {
  cat("📊 Gerando Biplot com Clusters...\n")
  ...
  # Carregar resultados de clusters
  clusters_data <- read.csv(cluster_file)
  
  # Extrair scores do PCA
  pca_scores <- as.data.frame(pca_result$pca$ind$coord[, 1:2])
  colnames(pca_scores) <- c("PC1", "PC2")
  
  # Adicionar clusters aos scores
  pca_scores$Cluster <- as.factor(clusters_data$Cluster_KMeans[1:nrow(pca_scores)])
  
  # Definir nomes e cores dos clusters
  cluster_names <- c(
  ...
  pca_scores$Cluster_Label <- cluster_names[as.character(pca_scores$Cluster)]
  
  cores_clusters <- c("#E41A1C", "#377EB8", "#4DAF4A", "#FF7F00", "#984EA3")
  
  # Criar visualização
  p <- ggplot(pca_scores, aes(x = PC1, y = PC2, color = Cluster_Label, fill = Cluster_Label)) +
    ...
    scale_fill_manual(values = cores_clusters, name = "Grupos Temáticos") +
    labs(
      title = "Análise PCA com Grupos Temáticos Identificados",
      subtitle = "Distribuição dos estudos no espaço de componentes principais",
      x = sprintf("Componente Principal 1 (%.1f%% da variância)", 
                  100 * pca_result$pca$eig[1, 2]),
      y = sprintf("Componente Principal 2 (%.1f%% da variância)", 
                  100 * pca_result$pca$eig[2, 2])
    ) +
  ...
  cat(sprintf("✓ Biplot com Clusters salvo: %s\n", output_file))
}
...
main <- function() {
  ...
  plot_biplot_with_clusters(pca_result)
