...
\begin{enumerate}
  \item Como métodos de \textit{machine learning} e processamento de linguagem natural podem mapear e classificar conhecimentos agroecológicos tradicionais a partir de narrativas, entrevistas e registros comunitários, com desempenho comparável ao de avaliadores especialistas?
  \item Quais algoritmos e representações (por exemplo, \textit{embeddings} semânticos, modelos baseados em tópicos, classificadores supervisionados) oferecem melhor equilíbrio entre desempenho, interpretabilidade e robustez em cenários de dados escassos e heterogêneos?
  \item Como co-desenhar, com participação ativa dos Povos e Comunidades Tradicionais (PCTs), um protocolo de validação participativa que respeite o consentimento livre, prévio e informado (CLPI), os direitos coletivos e as salvaguardas éticas?
  \item Quais métricas multidimensionais devem ser adotadas para avaliar o sistema (por exemplo, precisão técnica, relevância sociocultural, utilidade para gestão territorial e políticas públicas, risco de exposição de conhecimentos sensíveis) e como conciliá-las em um índice composto de validação?
  \item De que forma os produtos do mapeamento e da validação podem subsidiar instrumentos de salvaguarda e políticas (Lei nº 13.123/2015, SNUC — Lei nº 9.985/2000), inclusive para conhecimentos não diretamente vinculados a patrimônio genético?
  \item Que arranjos de governança de dados e mecanismos de proteção mitigam riscos de apropriação indevida e asseguram repartição justa de benefícios?
  \item Como garantir explicabilidade, detecção de vieses e calibração de incerteza ao longo do ciclo analítico, e qual o impacto disso na confiança, adoção e utilidade percebida pelas comunidades envolvidas?
\end{enumerate}

\subsection{Hipóteses}

\begin{enumerate}
  \item H1 (Desempenho técnico): Modelos de \textit{machine learning}/PLN treinados com dados comunitários e curadoria participativa atingem desempenho (F1 macro) $>$ 0{,}75 na classificação de temas agroecológicos validados por especialistas.
  \item H2 (Dados escassos): Métodos com representações semânticas (\textit{embeddings}) e ajuste fino supervisionado superam abordagens baseadas apenas em palavras-chave em cenários de dados escassos ($p < 0{,}05$).
  \item H3 (Validação participativa): A incorporação de validação participativa (CLPI, revisão comunitária e devolutivas) aumenta significativamente a relevância sociocultural percebida (escala Likert, d de Cohen $> 0{,}5$) sem reduzir o desempenho técnico do sistema.
  \item H4 (Métrica composta): Um índice composto de validação (precisão técnica, relevância sociocultural, utilidade para políticas e risco de exposição) apresenta consistência interna aceitável ($\alpha$ de Cronbach $> 0{,}70$) e estrutura fatorial coerente (EFA/CFA).
  \item H5 (Governança de dados): Protocolos bioculturais e licenças comunitárias reduzem a intenção de uso não autorizado dos dados (escala de intenção, $p < 0{,}05$) e aumentam a confiança das comunidades no sistema.
  \item H6 (Explicabilidade e vieses): Adoção de técnicas de explicabilidade (saliency, SHAP) e detecção de vieses reduz erros sistemáticos entre grupos (diferença absoluta de taxas $< 10\%$) e eleva a confiança dos participantes ($p < 0{,}05$).
  \item H7 (Escala psicométrica): A escala psicométrica proposta para registro e salvaguarda de saberes apresenta validade convergente (AVE $> 0{,}50$), confiabilidade composta ($> 0{,}70$) e ajuste de modelo satisfatório (CFI $> 0{,}90$; RMSEA $< 0{,}08$).
  \item H8 (Análise espacial): Práticas agroecológicas mapeadas apresentam associação espacial significativa com características ambientais do território (Moran’s I, $p < 0{,}05$), indicando padrões territoriais relevantes para gestão.
  \item H9 (Redes de circulação): Redes de circulação de saberes e produtos exibem propriedades de pequeno mundo/alta centralidade que se correlacionam com maior resiliência e difusão de práticas (coeficientes de correlação, $p < 0{,}05$).
  \item H10 (Integração em políticas): Produtos do sistema (mapas, tipologias e evidências) aumentam a utilidade percebida por formuladores de políticas e gestores de UCs (escala de utilidade), favorecendo sua adoção em instrumentos de gestão ($p < 0{,}05$).
\end{enumerate}
...
Desenvolver um modelo empírico por meio de machine learning que sirva como uma matriz de indicadores para salvaguarda dos saberes agroecológicos dos povos originários e comunidades tradicionais do Território de Identidade Semiárido Nordeste II na Bahia.
\subsection{Objetivos específicos}

\begin{itemize}
  \item Mapear e classificar, com ML/PLN, conhecimentos agroecológicos tradicionais a partir de entrevistas, narrativas e registros comunitários, visando desempenho $\mathrm{F1}_{\text{macro}} > 0{,}75$ (H1).
  \item Comparar algoritmos e representações (\textit{embeddings}, tópicos, classificadores) sob dados escassos/heterogêneos, identificando a combinação com melhor equilíbrio entre desempenho e interpretabilidade ($p < 0{,}05$) (H2).
  \item Co-desenhar, com PCTs, um protocolo de validação participativa que incorpore CLPI, revisão comunitária e devolutivas, e avaliar impacto na relevância sociocultural (d de Cohen $> 0{,}5$) (H3).
  \item Propor e estimar um índice composto de validação (precisão técnica, relevância sociocultural, utilidade para políticas, risco de exposição) com consistência interna ($\alpha > 0{,}70$) e estrutura fatorial válida (EFA/CFA) (H4).
  \item Elaborar e validar uma escala psicométrica para registro/salvaguarda de saberes (AVE $> 0{,}50$, confiabilidade composta $> 0{,}70$, CFI $> 0{,}90$, RMSEA $< 0{,}08$) (H7).
  \item Implementar governança de dados (protocolos bioculturais, licenças comunitárias, repositório comunitário) e medir redução de intenção de uso não autorizado e aumento de confiança ($p < 0{,}05$) (H5).
  \item Incorporar explicabilidade (saliency, SHAP), detecção de vieses e calibração de incerteza nos modelos, avaliando redução de disparidades ($< 10\%$) e aumento de confiança ($p < 0{,}05$) (H6).
  \item Realizar análise espacial (ex.: Moran’s I, LISA) para identificar padrões e associações entre práticas e variáveis ambientais do território (H8).
  \item Caracterizar redes de circulação de saberes/produtos (centralidades, modularidade, pequeno mundo) e associá-las à difusão e resiliência (H9).
  \item Produzir mapas, tipologias e relatórios que subsidiem políticas (Lei 13.123/2015; SNUC), avaliando utilidade percebida por gestores/UCs e propensão de adoção ($p < 0{,}05$) (H10).
  \item Documentar multimodalmente (texto, imagem, áudio/vídeo) práticas e contextos, assegurando consentimento e salvaguardas éticas, para compor repositório comunitário interoperável.
  \item Conduzir retroalimentação contínua e validação comunitária das saídas (oficinas, painéis deliberativos), integrando revisões ao ciclo de desenvolvimento do sistema.
\end{itemize}

\subsection{Plano de análise}

O plano de análise alinha cada hipótese e objetivo específico às técnicas analíticas e métricas de avaliação correspondentes, estruturando um quadro metodológico integrado que assegura a coerência entre os pressupostos teóricos, as questões de pesquisa e os instrumentos de validação empírica.

\subsubsection*{Mapeamento de análises por hipótese}

\textbf{H1 (ML/PLN para classificação de conhecimentos):}
\begin{itemize}
  \item \textit{Pipeline de ML}: Pré-processamento de texto (tokenização, lematização, remoção de \textit{stopwords}), extração de características (TF-IDF, \textit{embeddings} Word2Vec/GloVe/BERT), treino de classificadores supervisionados (SVM, Random Forest, redes neurais).
  \item \textit{Métricas}: F1 macro, precisão, recall, acurácia, matriz de confusão, curvas ROC-AUC.
  \item \textit{Validação}: Validação cruzada k-fold, divisão temporal dos dados, avaliação por especialistas externos.
\end{itemize}

\textbf{H2 (Comparação de algoritmos em cenários de dados escassos):}
\begin{itemize}
  \item \textit{Análise comparativa}: Teste t pareado ou ANOVA para comparação de desempenho entre algoritmos, análise de \textit{bootstrap} para intervalos de confiança.
  \item \textit{Métricas de interpretabilidade}: LIME, SHAP para explicabilidade local, importância de características, análise de erros por categoria.
  \item \textit{Robustez}: Testes de perturbação de dados, análise de sensibilidade, validação em domínios cruzados.
\end{itemize}

\textbf{H3 (Validação participativa e relevância sociocultural):}
\begin{itemize}
  \item \textit{Desenho experimental}: Pré/pós-teste com grupo controle, medidas repetidas para avaliar mudanças ao longo do tempo.
  \item \textit{Instrumentos}: Escalas Likert para relevância sociocultural, questionários de satisfação, grupos focais.
  \item \textit{Análise}: Teste t para amostras pareadas, d de Cohen para tamanho do efeito, análise temática qualitativa dos \textit{feedbacks}.
\end{itemize}

\textbf{H4 (Índice composto de validação):}
\begin{itemize}
  \item \textit{Análise Fatorial Exploratória (EFA)}: Identificação da estrutura latente, análise de componentes principais, rotação Varimax/Promax.
  \item \textit{Análise Fatorial Confirmatória (CFA)}: Modelagem de equações estruturais (SEM), índices de ajuste (CFI, TLI, RMSEA, SRMR).
  \item \textit{Confiabilidade}: α de Cronbach, confiabilidade composta (CR), variância extraída média (AVE).
\end{itemize}

\textbf{H5 (Governança de dados e confiança):}
\begin{itemize}
  \item \textit{Análise experimental}: Desenho entre grupos para comparar condições com/sem protocolos bioculturais.
  \item \textit{Métricas}: Escalas de intenção comportamental, medidas de confiança institucional, análise de regressão logística.
  \item \textit{Testes estatísticos}: Qui-quadrado para variáveis categóricas, ANOVA para variáveis contínuas, modelos de regressão múltipla.
\end{itemize}

\textbf{H6 (Explicabilidade, vieses e calibração):}
\begin{itemize}
  \item \textit{Detecção de vieses}: Análise de disparidade entre grupos demográficos, teste de equidade algorítmica (\textit{demographic parity}, \textit{equalized odds}).
  \item \textit{Calibração}: Gráficos de calibração, teste de Hosmer-Lemeshow, \textit{Brier score} para confiabilidade probabilística.
  \item \textit{Explicabilidade}: Implementação de SHAP, LIME, \textit{saliency maps}, questionários de compreensibilidade pelos usuários.
\end{itemize}

\textbf{H7 (Escala psicométrica para salvaguarda):}
\begin{itemize}
  \item \textit{Desenvolvimento}: Teoria de Resposta ao Item (TRI), análise de funcionamento diferencial de itens (DIF).
  \item \textit{Validação}: EFA e CFA, análise multigrupo, invariância de medida, validade convergente e discriminante.
  \item \textit{Métricas}: AVE, CR, α de Cronbach, índices de ajuste do modelo (CFI > 0,90; RMSEA < 0,08).
\end{itemize}

\textbf{H8 (Análise espacial de práticas agroecológicas):}
\begin{itemize}
  \item \textit{Autocorrelação espacial}: Índice de Moran's I global e local (LISA), testes de significância, \textit{hot spots} e \textit{cold spots}.
  \item \textit{Regressão espacial}: Modelos de defasagem espacial (SAR), modelos de erro espacial (SEM), regressão geograficamente ponderada (GWR).
  \item \textit{Ferramentas}: Sistemas de Informações Geográficas (SIG), R (pacotes \textit{spdep}, \textit{spatstat}), GeoDa.
\end{itemize}

\textbf{H9 (Redes de circulação de saberes):}
\begin{itemize}
  \item \textit{Métricas de rede}: Centralidade de grau, intermediação e proximidade, coeficiente de \textit{clustering}, modularidade, distância média.
  \item \textit{Detecção de comunidades}: Algoritmos de Louvain, \textit{walktrap}, análise de blocos estocásticos.
  \item \textit{Análise de correlação}: Pearson/Spearman entre métricas de rede e indicadores de resiliência, regressão múltipla, modelagem multinível.
\end{itemize}

\textbf{H10 (Integração em políticas públicas):}
\begin{itemize}
  \item \textit{Avaliação de utilidade}: Escalas de utilidade percebida, propensão de adoção, análise de aceitação tecnológica (TAM).
  \item \textit{Análise de adoção}: Modelos de regressão logística, análise de sobrevivência (tempo até adoção), análise de mediação.
  \item \textit{Avaliação de impacto}: Desenhos quasi-experimentais, pareamento por escore de propensão, diferença-em-diferenças.
\end{itemize}

\subsubsection*{Cronograma de análises e marcos de validação}

As análises serão conduzidas em fases sequenciais e iterativas:
\begin{enumerate}
  \item \textit{Fase 1 (Meses 1-6)}: Coleta e pré-processamento de dados, análises exploratórias, desenvolvimento do \textit{pipeline} de ML.
  \item \textit{Fase 2 (Meses 7-12)}: Implementação de modelos preditivos, validação participativa, análise espacial.
  \item \textit{Fase 3 (Meses 13-18)}: Análise de redes, desenvolvimento da escala psicométrica, avaliação de vieses e calibração.
  \item \textit{Fase 4 (Meses 19-24)}: Integração dos resultados, validação final com \textit{stakeholders}, preparação de produtos para políticas públicas.
\end{enumerate}

Cada marco será acompanhado de relatórios técnicos, validação comunitária e revisão por pares, assegurando a triangulação metodológica e a credibilidade dos achados.


\section{Justificativa}

A presente pesquisa justifica-se pela necessidade de superar as lacunas epistemológicas, metodológicas e institucionais que ainda limitam o reconhecimento efetivo dos saberes agroecológicos tradicionais como parte integrante do patrimônio imaterial e científico da humanidade. Embora os Povos e Comunidades Tradicionais (PCTs) desempenhem papel crucial na conservação da biodiversidade, na gestão sustentável dos ecossistemas e na manutenção da resiliência socioambiental, seus conhecimentos permanecem sub-representados nos sistemas formais de ciência, tecnologia e inovação (Mistry e Berardi, 2016).

A crise ecológica contemporânea, agravada pelos processos de degradação ambiental e pela homogeneização cultural, exige novas abordagens que conciliem o rigor técnico da ciência com o reconhecimento da pluralidade epistemológica. Nesse sentido, compreender, registrar e validar os conhecimentos agroecológicos tradicionais não é apenas uma tarefa científica, mas também um ato político e ético, pois reafirma o direito dessas comunidades à autodeterminação cognitiva, à propriedade intelectual e à gestão de seus territórios.

Apesar da existência de marcos legais como a Lei nº 13.123/2015, que regula o acesso ao patrimônio genético e ao conhecimento tradicional a ele associado, e do Sistema Nacional de Unidades de Conservação da Natureza (SNUC, Lei nº 9.985/2000), ainda persistem insuficiências na proteção e salvaguarda de práticas e saberes não diretamente vinculados a recursos genéticos. Essas lacunas comprometem a implementação de políticas públicas inclusivas e dificultam a incorporação do conhecimento tradicional em estratégias de desenvolvimento rural sustentável e de conservação socioambiental.

No contexto do Território de Identidade Semiárido Nordeste II, localizado no estado da Bahia, a relevância dessa discussão é ainda maior. Essa região abriga comunidades quilombolas, indígenas e camponesas que desenvolveram, ao longo de gerações, sistemas de manejo adaptados à escassez hídrica, à variabilidade climática e às especificidades do solo e da vegetação da Caatinga. Esses sistemas, baseados na observação empírica e na experimentação contínua, configuram verdadeiros sistemas de conhecimento adaptativo, capazes de oferecer soluções inovadoras para desafios globais como a desertificação, a insegurança alimentar e a perda de biodiversidade.

Entretanto, a documentação e a validação desses saberes enfrentam obstáculos substanciais, como a fragmentação dos registros, a ausência de metodologias sistemáticas de análise e o risco de apropriação indevida de informações sensíveis. A adoção de métodos baseados em machine learning e em processamento de linguagem natural (PLN) representa uma inovação metodológica significativa, pois permite organizar, mapear e interpretar grandes volumes de narrativas e registros comunitários com precisão, transparência e reprodutibilidade científica.

A proposta de um modelo empírico de salvaguarda construído de forma participativa e ancorado em princípios éticos, culturais e tecnológicos busca preencher a lacuna existente entre o conhecimento local e as ferramentas contemporâneas de gestão. Ao co-desenhar o sistema juntamente com as próprias comunidades, o projeto reforça o paradigma da ciência cidadã e da pesquisa colaborativa, assegurando o consentimento livre, prévio e informado (CLPI) e o respeito aos protocolos bioculturais.

Do ponto de vista científico, a originalidade da pesquisa reside na combinação entre técnicas de machine learning, métricas psicométricas e validação participativa aplicadas ao registro de saberes tradicionais, um campo ainda incipiente no Brasil e no cenário internacional. A abordagem proposta visa à construção de um índice composto de validação que integre dimensões técnicas, socioculturais e ético-políticas, oferecendo um modelo replicável para outros territórios e contextos culturais.

Sob a perspectiva social, a pesquisa contribui para a valorização dos PCTs como sujeitos epistêmicos e agentes de desenvolvimento sustentável, fortalecendo sua autonomia e ampliando a difusão de suas práticas agroecológicas. Ao registrar e validar esses conhecimentos de forma tecnicamente robusta e culturalmente sensível, o estudo promove a justiça cognitiva, entendida como a equidade no reconhecimento e na circulação de saberes, e oferece subsídios concretos para políticas públicas de conservação, gestão territorial e inovação sustentável.

A relevância deste estudo manifesta-se em três dimensões complementares: científica, ao propor uma integração inédita entre inteligência artificial e epistemologias tradicionais; social e política, ao fortalecer os direitos e o protagonismo dos PCTs na governança de seus saberes e territórios; e ambiental, ao reconhecer esses sistemas de conhecimento como componentes centrais das estratégias de adaptação e conservação da biodiversidade no Semiárido brasileiro.

Assim, esta tese insere-se no esforço global de descolonização da ciência, ampliando o diálogo entre conhecimento tradicional e tecnologias emergentes, e contribuindo para a construção de modelos mais justos, inclusivos e sustentáveis de desenvolvimento territorial.
...
\begin{enumerate}
  \item \textbf{Categoria 1.} São analisados e classificados automaticamente nesta categoria os artigos categorizados nos quadrantes Q1 e Q4, onde o quadrante Q1 inclui automaticamente todos os artigos já analisados e aceitos e o quadrante Q4 exclui automaticamente todos os artigos já analisados e excluídos;

   \item \textbf{Categoria 2.} São analisados e classificados automaticamente nesta categoria os artigos categorizados nos quadrantes Q2 e Q3, onde o quadrante Q2 inclui automaticamente artigos a serem aceitos manualmente pelo avaliador, já para o Q3, o sistema inclui os artigos rejeitados por pontuação baixa mais passiveis de inclusão no Q2 e posterior seleção manual pelo avaliador.
\end{enumerate}
...
\begin{subfigure}{0.23\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/logo_mahabio.pdf}
  \caption{Maha Biocosméticos}
  \label{fig:logos-mahabio}
\end{subfigure}
\begin{subfigure}{0.23\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/logo_de_mendes.pdf}
  \caption{De Mendes}
  \label{fig:logos-demendes}
\end{subfigure}
\begin{subfigure}{0.23\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/logo_uika.pdf}
  \caption{UIKA}
  \label{fig:logos-uika}
\end{subfigure}
\begin{subfigure}{0.23\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/logo_fazenda_bacuri.pdf}
  \caption{Fazenda Bacuri}
  \label{fig:logos-bacuri}
\end{subfigure}

\begin{subfigure}{0.23\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/logo_muriki.pdf}
  \caption{Muriki}
  \label{fig:logos-muriki}
\end{subfigure}
\begin{subfigure}{0.23\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/logo_osaqui.pdf}
  \caption{Osaqui}
  \label{fig:logos-osaqui}
\end{subfigure}
\begin{subfigure}{0.23\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/logo_santa_barbara.pdf}
  \caption{Cogumelos Santa Bárbara}
  \label{fig:logos-santabarbara}
\end{subfigure}
\begin{subfigure}{0.23\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/logo_xibe.pdf}
  \caption{Xibé}
  \label{fig:logos-xibe}
\end{subfigure}
...
\begin{subfigure}{0.45\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_27.pdf}
  \caption{Biscoito de castanha do Pará com puxuri}
  \label{fig:demendes-biscoito}
\end{subfigure}
\begin{subfigure}{0.45\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_28.pdf}
  \caption{Barra de chocolate Acará-Açu}
  \label{fig:demendes-barra}
\end{subfigure}
...
\begin{subfigure}{0.45\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_29.pdf}
  \caption{Fruta seca bacuri}
  \label{fig:osaqui-frutaseca}
\end{subfigure}
\begin{subfigure}{0.45\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_26.pdf}
  \caption{Licor de Genipapo}
  \label{fig:osaqui-licor}
\end{subfigure}
...
\begin{subfigure}{0.45\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_30.pdf}
  \caption{Artesanatos comercializados nas comunidades}
  \label{fig:uika-artesanatos}
\end{subfigure}
\begin{subfigure}{0.45\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_31.pdf}
  \caption{Comunidades ribeirinhas-AM}
  \label{fig:uika-ribeirinhas}
\end{subfigure}
...
\begin{subfigure}{0.48\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_32.pdf}
  \caption{Trilhas ecológicas}
  \label{fig:muriki-trilhas}
\end{subfigure}
\begin{subfigure}{0.48\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_34.pdf}
  \caption{Experiências locais}
  \label{fig:muriki-locais}
\end{subfigure}
\begin{subfigure}{0.48\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_35.pdf}
  \caption{Experiências comunitárias}
  \label{fig:muriki-comunitarias}
\end{subfigure}
\begin{subfigure}{0.48\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_33.pdf}
  \caption{Eventos esportivos}
  \label{fig:muriki-eventos}
\end{subfigure}
...
\begin{subfigure}{0.23\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_36.pdf}
  \caption{Tinturas naturais}
  \label{fig:xibe-tinturas}
\end{subfigure}
\begin{subfigure}{0.23\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_37.pdf}
  \caption{Lenços pashmina}
  \label{fig:xibe-lencos}
\end{subfigure}
\begin{subfigure}{0.23\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_38.pdf}
  \caption{Acessórios artesanais}
  \label{fig:xibe-acessorios}
\end{subfigure}
\begin{subfigure}{0.23\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_39.pdf}
  \caption{Bolsas}
  \label{fig:xibe-bolsas}
\end{subfigure}
\caption{Produtos e peças desenvolvidas pela Xibé}
\fonte{\citeonline{Muriki2023MurikiCicloturismo}}
\end{figure}
...
\begin{subfigure}{0.45\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_43.pdf}
  \caption{Visão interna da estufa}
  \label{fig:cogumelos-estufa-interna}
\end{subfigure}
\begin{subfigure}{0.45\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_44.pdf}
  \caption{Visão externa da estufa}
  \label{fig:cogumelos-estufa-externa}
\end{subfigure}
...
\begin{subfigure}{0.32\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_40.pdf}
  \caption{Condicionador de Andiroba}
  \label{fig:mahabio-condicionador}
\end{subfigure}
\begin{subfigure}{0.32\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_41.pdf}
  \caption{Xampu de Castanha-do-Pará}
  \label{fig:mahabio-xampu}
\end{subfigure}
\begin{subfigure}{0.32\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Imagens/fig_42.pdf}
  \caption{Creme de Ucuuba}
  \label{fig:mahabio-creme}
\end{subfigure}
...
\begin{enumerate}[label=(\roman*)]
\item Produzir um instrumento composto de conhecimento científico e uma ferramenta ativável;
