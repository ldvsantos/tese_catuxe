...
Essas barreiras não são meramente acadêmicas. A literatura sobre equivalência transcultural \cite{Herdman1999,Guillemin1993} indica que a aplicação de instrumentos sem adaptação formal tende a produzir viés sistemático de mensuração. No contexto desta tese, esse risco assume magnitude crítica: o WOCAT-SLM servirá como template de referência para a elicitação Delphi e, indiretamente, para a construção do Índice de Salvaguarda Biocultural (ISB) via modelagem fuzzy (OE4). Um instrumento mal adaptado propagaria erros sistêmicos por toda a cadeia.
...
Conforme evidenciado pela revisão de escopo PRISMA-ScR conduzida no Capítulo 1, embora o Aprendizado de Máquina alcance acurácias combinadas superiores a 90\% no monitoramento de paisagens agrícolas, a conformidade FAIR permanece criticamente baixa e a ausência de protocolos de validação espacial compromete a prontidão operacional. Antes de se aplicar qualquer tecnologia computacional sofisticada, é necessário dispor de variáveis de entrada estruturadas, validadas e contextualmente pertinentes.
...
\item \textbf{Validade baseada no conteúdo:} aferida pelo IVC e kappa de Fleiss (Etapa~4), complementados pela taxa de compreensão do pré-teste (Etapa~5). Itens com $IVC < 0{,}60$ serão eliminados e itens com $0{,}60 \leq IVC < 0{,}80$ serão revisados pelo comitê.

\item \textbf{Validade baseada na estrutura interna:} após aplicação do instrumento adaptado, será conduzida análise fatorial confirmatória (AFC) para verificar o ajuste do modelo de mensuração às dimensões teóricas ($V_1$--$V_8$). Serão avaliados os índices de ajuste CFI ($\geq 0{,}90$), TLI ($\geq 0{,}90$), RMSEA ($\leq 0{,}08$) e SRMR ($\leq 0{,}08$) \cite{Brown2015,Kline2016PrinciplesLivros}. Cargas fatoriais padronizadas $\geq 0{,}40$ serão consideradas aceitáveis. A consistência interna de cada dimensão será estimada pelo coeficiente ômega de McDonald ($\omega \geq 0{,}70$) e pela fidedignidade composta \cite{Raykov2016EstimationMeasures}.

\item \textbf{Validade baseada nas relações com variáveis externas:} as pontuações do instrumento adaptado serão correlacionadas com indicadores externos disponíveis (e.g., índice de agrobiodiversidade, cobertura fundiária, indicadores socioeconômicos municipais), verificando validade convergente e discriminante mediante variância média extraída ($AVE \geq 0{,}50$) e critério de Fornell-Larcker.

\item \textbf{Validade baseada no padrão de respostas:} distribuições das respostas por item serão inspecionadas para efeitos de piso e teto ($\leq 15\%$ em qualquer categoria extrema como limiar), curtose e assimetria. Itens com distribuição degenerada serão reavaliados qualitativamente.
\end{enumerate}

As análises de AFC e fidedignidade serão conduzidas em R versão 4.5.1 \cite{RCore2024}, empregando o pacote \texttt{lavaan} com estimador WLSMV para dados ordinais \cite{Sellbom2019FactorRecommendations} e o pacote \texttt{semTools} para cômputo de $\omega$ e $AVE$. Essa etapa de validação assegura que o instrumento adaptado disponha de evidências psicométricas robustas antes de alimentar as etapas subsequentes de modelagem fuzzy.
