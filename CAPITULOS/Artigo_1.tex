...
No entanto, o desempenho é heterogêneo e depende do rigor metodológico. Acurácias de 100\% em classificação binária (ex: presunto Jinhua, chá Wuyi Rock) frequentemente refletem alta separabilidade de classes via marcadores únicos\cite{Chen2020; Effrosynidis2021}. Tais resultados justificam ceticismo se não acompanhados de validação externa robusta.

Uma faixa de desempenho mais realista para problemas multiclasse (ex: múltiplas denominações de vinho) é 88\%–99\%. Estudos alcançando alta sensibilidade (>99,3\%) frequentemente empregam validação rigorosa como k-fold repetido ou leave-one-out [@; \cite{Meena2024; Mohammadi2024}4]. Na detecção de fraude (54\% dos estudos), o desempenho é priorizado via sensibilidade e especificidade para minimizar falsos negativos, frequentemente usando técnicas de balanceamento de classe.
...
Na modelagem preditiva global, tarefas de regressão mostraram desempenho modesto (RMSE 11,8–12,5, $R^2$ 0,11–0,14), indicando que prever um índice de qualidade sintético a partir de variáveis bibliométricas é difícil \cite{Hair2010; Wang2013}. Por outro lado, tarefas de classificação (alta pontuação vs. outros) mostraram que a Regressão Logística superou o Random Forest (Acurácia 0,69 vs. 0,53), sugerindo que estudos de alta qualidade são distinguidos por padrões relativamente simples e lineares \cite{Rudin2019}. A Figura 6 destaca essa assimetria.
