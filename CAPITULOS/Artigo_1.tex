\chapter{Machine Learning in Geographical Indication Certification and Authentication: A Scoping Review}
...
\begin{tabular}{|l|l|}
\hline
\textbf{P (População)} & Indicações Geográficas, Denominações de Origem e Indicações de Procedência reconhecidas nacional e internacionalmente, abrangendo produtos agroalimentares (vinhos, queijos, cafés, carnes, azeites), artesanato e outros produtos com identidade territorial. \ \hline
\textbf{C (Conceito)} & Machine Learning, Inteligência Artificial, algoritmos de classificação e predição, métodos quimiométricos, Mineração de Dados e Processamento de Linguagem Natural aplicados a contextos de Indicações Geográficas. \ \hline
\textbf{C (Contexto)} & Autenticação de origem geográfica, avaliação de potencial de IG, identificação de determinantes territoriais (solo, clima, métodos de produção), classificação e discriminação de produtos, sistemas de apoio à decisão para certificação, controle de qualidade, rastreabilidade, detecção de fraudes e adulterações, e estratégias de valorização territorial. \ \hline
\end{tabular}
\end{table}
...
\textbf{Figura 1.} Fluxograma da Revisão de Escopo sobre Aplicações de Machine Learning em Indicações Geográficas.
...
\textbf{Tabela 2.} Indicadores de qualidade metodológica para estudos ML-IG.
...
IMP & Impacto e aplicabilidade dos resultados para sistemas de IG & Qualidade Social \ \hline
...
Esta fase resultou na seleção de 25 estudos com qualidade metodológica adequada (pontuação \geq 20 pontos) dos 272 artigos iniciais. Estes formaram a base para análises subsequentes. A distribuição incluiu 1 artigo de excelência (\geq40 pts), 2 de alta relevância (\geq30 pts) e 22 adequados (\geq20 pts).
...
\subsection{3.1 Síntese Executiva da Revisão de Escopo}
...
\textbf{Figura 2.} Fluxograma da Revisão de Escopo sobre Aplicações de Machine Learning em Indicações Geográficas.
...
\subsection{3.2 Análise Estrutural e Temporal do Corpus Científico}
...
\textbf{Figura 3.} Biplot da Análise de Correspondência Múltipla (MCA) sobre a evolução temporal das aplicações de ML para autenticação de Indicações Geográficas (2010-2025).

% \begin{figure}[h]
% \centering
\includegraphics[width=0.8\textwidth]{2-FIGURAS/2-EN/mca_biplot_temporal_completo.png}
% \caption{}
...
Vinhos com denominação de origem protegida (DOP) ,  como os do Douro, Rioja e Bordeaux ,  e chás com IG como Wuyi Rock Tea constituem o domínio de aplicação primário. Nesses estudos, a discriminação de origem baseia-se em \emph{fingerprinting} metabolômico e análise de traços elementares, demonstrando que o perfil químico está intrinsecamente ligado às condições geográficas de produção e ao \emph{terroir} \cite{Ramos2025; Xu2021}. Carnes e produtos cárneos (ex: cordeiro regional, presunto Jinhua) formam uma segunda categoria importante, onde a discriminação de origem é baseada em assinaturas elementares e isotópicas processadas por algoritmos como Random Forest e SVM \cite{Chen2020}.

Setores como frutas, vegetais e plantas medicinais expandem essa estrutura. Para frutas e vegetais, o ML rastreia a origem via \emph{fingerprints} metabólicos e perfis nutricionais, testando a hipótese de que assinaturas bioquímicas refletem condições edafoclimáticas específicas \cite{Luan2020; Peng2025}. Para plantas medicinais como \emph{Panax notoginseng}, a certificação de origem também está ligada à potência farmacológica, reforçando a conexão entre geografia, composição bioativa e valor econômico \cite{Feng2025}.

Geograficamente, instituições asiáticas, particularmente na China, dominam o cenário de pesquisa, seguidas pela Europa. O Brasil e outras economias emergentes estão menos representados. Essa assimetria reflete o recente investimento chinês em tecnologias de rastreabilidade \cite{Wang2025} versus a infraestrutura analítica consolidada na Europa. Para o Brasil, essa lacuna representa uma oportunidade estratégica para desenvolver aplicações de ML para proteger e valorizar IGs nacionais.
...
\subsection{3.4 O Ecossistema Algorítmico na Autenticação de IG}

O ecossistema algorítmico é diverso, mas estruturado em torno de núcleos tecnológicos específicos. Em vez de fragmentação, essa variedade indica que diferentes matrizes, tamanhos de amostra e contextos regulatórios exigem soluções computacionais personalizadas. Algoritmos de classificação supervisionada dominam, com Random Forest e Support Vector Machines (SVM) respondendo por aproximadamente dois terços das aplicações. Por exemplo, \cite{Xu2021} usaram Random Forest para discriminar a origem do vinho via perfis elementares (acurácia > 95\%), enquanto \cite{Mohammadi2024} integraram SVM com um kernel RBF para autenticar azeites usando espectroscopia NIR em um cenário de alta dimensionalidade.
...
A prevalência do Random Forest decorre de sua capacidade de modelar interações não lineares, lidar com classes desbalanceadas e fornecer Medidas de Importância de Variável (VIM) para identificar marcadores territoriais com valor regulatório \cite{Zhang2025MRF}. Esses modelos operam como comitês de classificadores $h_m(x)$, agregados em uma decisão final $\hat{y}$ \cite{Cornelio2019VORACE}. \cite{Li2025} utilizaram essa arquitetura para isolar subconjuntos de variáveis químicas que suportam transparentemente alegações de autenticidade geográfica.

Em domínios de imagem ou hiperespectrais, Deep Learning ,  especificamente Redes Neurais Convolucionais (CNNs) ,  é o padrão emergente. \cite{Peng2025} usaram CNNs para classificar chás com IG a partir de imagens hiperespectrais, enquanto \cite{Feng2025} aplicaram arquiteturas profundas para autenticação visual de plantas medicinais. A operação de convolução $(K * X)(i,j)$ permite que filtros extraiam padrões locais, reforçando que a autenticação depende de estruturas espaciais ou espectrais recorrentes codificando o \emph{terroir}, não apenas valores de intensidade pontuais.
...
A redução de dimensionalidade é um tema transversal. A PCA é usada em mais da metade dos estudos para pré-processamento, redução de ruído e visualização. \cite{Ramos2025} aplicaram PCA para condensar variáveis metabolômicas de vinho para modelagem Random Forest. Métodos de seleção de características como RF-RFE e Boruta são empregados em um terço dos trabalhos para identificar subconjuntos de variáveis discriminantes. \cite{Salam2021} mostraram que o Boruta reduziu elementos traço de 80 para 15 em estudos de carne sem perda de desempenho. Essa combinação de redução/seleção e classificadores robustos é essencial para controlar o \emph{overfitting} e definir conjuntos compactos de marcadores territoriais para protocolos oficiais.
...
\subsection{3.5 Desempenho Preditivo dos Modelos}
...
A predominância de validações \emph{in silico} e a escassez de testes longitudinais e espaciais minam a robustez legal dos modelos, necessitando de protocolos de validação rigorosos.
...
O conjunto de dados completo que suporta os resultados deste estudo, incluindo o corpus bibliográfico, scripts de análise e resultados intermediários, está publicamente disponível no repositório Open Science Framework (OSF) sob o DOI: \url{https://doi.org/10.17605/OSF.IO/2EKYQ}.
...
\subsubsection{Tabela A.1: Distribuição de Produtos Agroalimentares com Indicações Geográficas por Categoria}
...
\hline
\textbf{Categoria de Produto} & \textbf{Exemplos Específicos} & \textbf{Indicações Geográficas Primárias} & \textbf{Técnicas de ML Predominantes} & \textbf{Frequência Relativa} \ \hline
Vinhos e Bebidas Alcoólicas & Vinho tinto, vinho branco, rosé; destilados de frutas; vinagres & Douro, Rioja, Bordeaux, Denominação de Origem Controlada (DOC) & Random Forest, SVM, PLS-DA & 34\% \ \hline
Chás & Chá Wuyi Rock, Liupao, Oolong, Chá Verde & China (Fujian, Zhejiang, Yunnan) & NIR + PLS-DA, GC-MS + ML & 18\% \ \hline
Carnes Processadas & Cordeiro, Presunto, Carne Bovina & Jinhua (China), Cordeiro Europeu IGP, Carne Halal & Análise Elementar + SVM, Deep Learning & 15\% \ \hline
Frutas e Vegetais & Cítricos, Cebola Tropea, Frutas Vermelhas & Sicília, Calábria (Itália), Várias Regiões & Metabolômica + Random Forest, NIR & 12\% \ \hline
Plantas Medicinais & Panax notoginseng (Ginseng), Ervas Medicinais & Yunnan (China), Regiões da Ásia & Metabolômica Untargeted, CNN & 8\% \ \hline
Azeites & Azeite Extra Virgem, Azeite de Oliva & Região do Mediterrâneo, Itália, Espanha & Fingerprinting NIR, SVM & 8\% \ \hline
Mel & Mel Floral, Mel Silvestre & Lages (Brasil), Regiões da Europa & Espectrometria Elementar, PLS-DA & 5\% \ \hline
\end{tabular}
\end{table}

\emph{Fonte: Distribuição de produtos agroalimentares com Indicações Geográficas por categoria, regiões geográficas associadas, técnicas de Machine Learning predominantes e frequência relativa de estudos no corpus analisado (N=148).}

\subsubsection{Tabela A.2: Artigos Selecionados pelo Princípio de Pareto (80/20)}
...
\hline
\textbf{Posição} & \textbf{Artigo} & \textbf{Pontuação Combinada} & \textbf{Principais Contribuições} \ \hline
1 & Li et al. (2025) & 95,2 & Deep Learning para autenticação de chá chinês \ \hline
2 & Wang et al. (2025) & 92,8 & Blockchain + ML para rastreabilidade \ \hline
3 & Ramos et al. (2025) & 90,5 & Metabolômica untargeted em vinhos \ \hline
4 & Peng et al. (2025) & 88,9 & CNN para imagens hiperespectrais \ \hline
5 & Jiang et al. (2025) & 87,3 & Classificação multi-espectral \ \hline
6 & Xu et al. (2021) & 85,7 & Random Forest em perfis elementares \ \hline
7 & Chen et al. (2020) & 84,1 & SVM em carnes processadas \ \hline
8 & Mohammadi et al. (2024) & 82,6 & NIR + PLS-DA em azeites \ \hline
9 & Rebiai et al. (2022) & 81,2 & Espectroscopia em vinhos europeus \ \hline
10 & Feng et al. (2025) & 79,8 & Redes neurais em plantas medicinais \ \hline
\end{tabular}
\end{table}

\emph{Fonte: 10 artigos selecionados pelo princípio de Pareto (80/20) no corpus de 148 estudos.}

\subsubsection{Tabela A.3: Módulos Tecnológicos Identificados pela Análise de Comunidade Louvain}
...
\begin{tabular}{|l|l|l|l|l|}
\hline
\textbf{Módulo} & \textbf{Algoritmos Principais} & \textbf{Técnicas Analíticas} & \textbf{Produtos} & \textbf{Região Predominante} \ \hline
\textbf{M1} & Random Forest, Decision Tree, Gradient Boosting & Espectroscopia (NIR), Quimiometria & Vinho, Mel & África, Europa \ \hline
\textbf{M2} & SVM, KNN & Cromatografia (GC-MS, LC-MS, HPLC) & Carnes, Produtos Regionais & Ásia \ \hline
\textbf{M3} & Neural Networks, CNN, Deep Learning & Espectroscopia (NIR, FTIR), Sensores (e-nose) & Azeite, Queijo, Chá & Europa, Ásia \ \hline
\end{tabular}
\end{table}

\emph{Fonte: Três principais módulos tecnológicos identificados pela análise de comunidade Louvain aplicada à rede de coocorrência entre algoritmos, técnicas analíticas e produtos com indicação geográfica. A densidade interna de cada módulo indica a força das conexões entre seus componentes.}
...
\hline
\textbf{Cluster} & \textbf{Produto Principal} & \textbf{Técnica Analítica} & \textbf{Algoritmo ML} & \textbf{Aplicação} & \textbf{Região Predominante} \ \hline
1 & Mel & Espectroscopia NIR & SVM, KNN & Autenticação e detecção de fraude & Ásia \ \hline
2 & Queijo & Espectroscopia NIR & Redes Neurais & Discriminação de origem & Europa \ \hline
3 & Mel, Carnes & LC-MS, GC-MS & SVM, Random Forest, Árvores de Decisão & Autenticação e rastreabilidade & Ásia, Europa \ \hline
\end{tabular}
\end{table}

\emph{Fonte: Dez clusters identificados pela análise de cluster (k-means e hierárquica) baseada em produto, instrumento analítico, algoritmo e tipo de aplicação. Apenas os três clusters mais notáveis são detalhados aqui.}