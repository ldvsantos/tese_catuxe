...
A predisposição à adoção será adicionalmente avaliada pelo modelo UTAUT2 (\textit{Unified Theory of Acceptance and Use of Technology}) adaptado ao contexto de comunidades tradicionais \cite{Venkatesh2012}, aferindo-se expectativa de desempenho, expectativa de esforço, influência social, condições facilitadoras e motivação hedônica. A intenção de uso contínuo e o uso efetivo serão estimados por PLS-SEM, considerando-se o tamanho amostral reduzido inerente a comunidades rurais \cite{Hair2019b}.
...
O plano de manutenção local contemplará três componentes operacionais. A capacitação técnica será conduzida mediante oficinas presenciais com duração mínima de 16 horas, cobrindo operação do módulo de submissão, interpretação do mapa de conformidade e procedimentos de \textit{backup} local dos dossiês. A manutenção de infraestrutura seguirá modelo de responsabilidade compartilhada, no qual a associação comunitária local administrará o nó Hyperledger Fabric peer e a instância do módulo de auditoria, com suporte técnico remoto da equipe do projeto durante os primeiros 24 meses pós-implantação. A atualização do corpus normativo (alterações em portarias IPHAN, resoluções CGen ou diretrizes FAO-GIAHS) será incorporada mediante ciclos semestrais de recalibração das CPTs da Rede Bayesiana e do corpus de referência SBERT.
